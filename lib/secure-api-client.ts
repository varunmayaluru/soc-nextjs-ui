...
import { encryptPayload, decryptPayload, type EncryptedData } from "./encryption"
...
export const secureApi = {
   ...
   */
  get<T>(endpoint: string, options: RequestInit = {}): Promise<SecureApiResponse<T>> {
   ...
   */
  post<T>(endpoint: string, data: any, options: RequestInit = {}, encrypt = true): Promise<SecureApiResponse<T>> {
   ...
   */
  put<T>(endpoint: string, data: any, options: RequestInit = {}, encrypt = true): Promise<SecureApiResponse<T>> {
    return secureApiRequest<T>(
      endpoint,
      {
        ...options,
   ...
   * DELETE request
   */
  delete<T>(endpoint: string, options: RequestInit = {}): Promise<SecureApiResponse<T>> {
    return secureApiRequest<T>(endpoint, { ...options, method: "DELETE" }, false)
  },

  /**
   * PATCH request with encryption
   */
  patch<T>(endpoint: string, data: any, options: RequestInit = {}, encrypt = true): Promise<SecureApiResponse<T>> {
    return secureApiRequest<T>(
      endpoint,
      {
        ...options,
